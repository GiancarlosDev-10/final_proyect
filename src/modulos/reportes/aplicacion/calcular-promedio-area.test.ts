...
describe("calcularPromedioArea", () => {
  it("agrega el promedio de todos los cursos del área, ignorando cursos de otras áreas", async () => {
    ...
    const unidadRepo = new FakeUnidadDidacticaRepositorio([
      ...
      crearUnidadDidactica({ id: "UDI-3", cursoId: "CUR-2", periodoId: "PER-1", orden: 1 }),
      crearUnidadDidactica({ id: "UDI-4", cursoId: "CUR-2", periodoId: "PER-1", orden: 2 }),
    ]);
    const notaRepo = new FakeNotaRepositorio([
      // Álgebra: Unidad1=15, Unidad2=18 -> bimestral 16.5
      ...
      // Aritmética: Unidad1=10, sin nota en Unidad2 -> bimestral 10
      crearNota({ id: "NOT-4", asignacionId: "AS-2", unidadDidacticaId: "UDI-3", valor: 10 }),
    ...
    expect(resultado.value.promediosPorUnidad).toEqual([
      { orden: 1, promedio: 12.5 }, // avg(15, 10)
      { orden: 2, promedio: 18 }, // solo Álgebra tiene nota ahí
    ]);
  ...
  it("retorna listas vacías y promedio null si el área no tiene cursos", async () => {
    ...
    expect(resultado.value.promediosPorUnidad).toEqual([
      { orden: 1, promedio: null },
      { orden: 2, promedio: null },
    ]);
